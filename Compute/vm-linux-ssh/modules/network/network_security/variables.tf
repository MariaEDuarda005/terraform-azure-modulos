...
variable "network_security_group_id" {
  ...
  validation {
    condition     = can(regex("^/subscriptions/.+/resourceGroups/.+/providers/Microsoft.Network/networkSecurityGroups/.+$", var.network_security_group_id))
    error_message = "The network security group ID must follow the Azure resource ID format."
  }
}
